import { JSBI, Pair, Percent, TokenAmount } from '@liuxingfeiyu/byobu-sdk'
...
export default function FullPositionCard({ pair, border, stakedBalance }: PositionCardProps) {
  ...
  const [token0WithLogo,token1WithLogo] =useMemo( ()=>{
    return [ allTokens[pair.token0.address],allTokens[pair.token1.address]]
  },[pair.token0,pair.token1])


  return (
    <div className="s-xyuzu-tab-wrapper" style={{margin: '5px 20px' }}>
      <StyledPositionCard border={border} bgColor={'#191B25'}>
        <AutoColumn gap="12px">
          <FixedHeightRow>
            <AutoRow gap="8px">
              <DoubleCurrencyLogo currency0={token0WithLogo} currency1={token1WithLogo} size={20} />
              <Text fontWeight={500} fontSize={20} color={'#CCDBE5'}>
                {!currency0 || !currency1 ? (
                  <Dots>{t('loading')}</Dots>
                ) : (
                  `${currency0.getSymbol(chainId)}/${currency1.getSymbol(chainId)}`
            ...
            <RowFixed gap="8px">
              ...
              >
                {showMore ? (
                  <>
                    {t('manage')}
                    <img src={ChevronDownImg} width="12px" style={{ marginLeft: '10px', transform:' rotate(180deg)' }} />
                  </>
                ) : (
                  <>
                    {t('manage')}
                    <img src={ChevronDownImg} width="12px" style={{ marginLeft: '10px' }} />
                  </>
                )}
              </ButtonEmpty>
            </RowFixed>
          ...
          {showMore && (
            <AutoColumn gap="8px">
              <FixedHeightRow>
                <Text fontSize={16} fontWeight={500} color={'#798796'}>
                {t('myLpBalance')}:
                </Text>
                <Text fontSize={16} fontWeight={500} color='#CCDBE5'>
                  {userPoolBalance ? stringFix(userPoolBalance.toSignificant(4), 4) : '-'}
                </Text>
              </FixedHeightRow>
              {stakedBalance && (
              ...
              <FixedHeightRow>
                ...
                {token0Deposited ? (
                  <RowFixed>
                    <Text fontSize={16} fontWeight={500} marginLeft={'6px'} color='#CCDBE5'>
