...
//import { ZERO } from '@liuxingfeiyu/byobu-sdk//constants'
...
import {ChainId, CurrencyAmount, JSBI, Token, TokenAmount ,StakePool, ZERO} from '@liuxingfeiyu/byobu-sdk'
        ...
        <div className="s-boardroom-item-details">
          <div className="s-boardroom-detail">
            <BoardRoomDetail>
              ...
              <p style={{color: '#CCDBE5'}}>{ fixFloat(prodPerBlock * UserRatioOfReward, 2)} YUZU</p>
            </BoardRoomDetail>
            <BoardRoomDetail>
              <p>{t('totalLp')}:</p>
              <p style={{color: '#CCDBE5'}}>{ transToThousandth(fixFloat(tvl, 4))} USDT</p>
            </BoardRoomDetail>
            <BoardRoomDetail>
              <p>{t('myStaked')}:</p> 
              <p style={{color: '#CCDBE5'}}>{ fixFloat(myRatio * 100, 2)}%</p>
            </BoardRoomDetail>
            <BoardRoomDetail>
              <p>{t('myReward')}:</p> 
              <p style={{color: '#CCDBE5'}}>{ fixFloat(myReward, 4)} YUZU</p>
            </BoardRoomDetail> 
            <BoardRoomDetail>
              <p>APR<QuestionHelper text={'This number is estimated given the assumption that each block time is 6s.'}/>：</p>
              <p style={{color: '#10A6BF'}}>{fixFloat(Apr, 3)}%</p>
            </BoardRoomDetail>         
          </div>
...
`
  ...
  const TimeBLock = styled.div<{notZero : boolean}>`
    ...
    line-height: 18px;
    color: ${({notZero, theme})=>(notZero ? theme.bg7 :  '#D0D0D0')};
    min-width: 20px;
  `
  ...
  return (
    <div className="s-doubleget-item">
      <div className="s-doubleget-item-con">
        ...
        <span>&nbsp;&nbsp;</span>
        { <h3 style={{display: 'inline-block', margin: '0px auto', verticalAlign: 'middle', lineHeight: '34px', color: '#CCDBE5'}}>{pool.token0.symbol}{!xyuzu && '-' + pool.token1.symbol}</h3> }
        </Link>
        ...
        <div className="s-doubleget-item-detail">
          ...
          <div className="s-doubleget-item-detail">
            ...
            <em style={{color:'#10A6BF'}}>{fixFloat(Apr, 3)}%</em>
          </div>
        </div>
