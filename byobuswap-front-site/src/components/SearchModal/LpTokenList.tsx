...
import { Currency, CurrencyAmount, currencyEquals, ETHER, Token , StakePool, JSBI} from '@liuxingfeiyu/byobu-sdk'
  ...
  return (
    ...
    >
      ...
      <p style={{ justifySelf: 'flex-end' }}>
        {balance ? 
            <Text >{fixFloatFloor(JSBI.toNumber(balance)/1e18, 8)}
            </Text>
         : account ? <Loader /> : null}
      </p>
    </LPMenuItem>
...
export default function LpTokenList({
  ...
  const itemData: (StakePool | undefined)[] = useMemo(() => {
    let formatted: (StakePool | undefined)[] = pools
    return formatted
  }, [pools])
